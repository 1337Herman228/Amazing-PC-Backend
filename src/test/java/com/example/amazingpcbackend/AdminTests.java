...
public class AdminTests {
    ...
    public void testAddPartition_Success() throws Exception {
        ...
        partition.setId("test-id");
        partition.setLabel("Test Partition");
        partition.setValue("test");

        // Преобразуем объект в JSON
        String requestBody = objectMapper.writeValueAsString(partition);

        // Выполняем POST-запрос к контроллеру
        ResultActions result = mockMvc.perform(post("/admin/partitions")
    ...
    public void testEditPartition_Success() throws Exception {
        ...
        Partitions partition = partitionsRepository.findByValue("test").get();

        // Редактируем название
        partition.setLabel("Edited Partition");

        // Преобразуем объект в JSON
        String requestBody = objectMapper.writeValueAsString(partition);

        // Выполняем PUT-запрос к контроллеру
        ResultActions result = mockMvc.perform(put("/admin/partitions")
                .contentType("application/json")
                .header("Authorization", "Bearer " + token)  // Добавляем токен в заголовок
                .content(requestBody));
    ...
    public void testDeletePartition_Success() throws Exception {
        ...
        String partitionIdToDelete = "test-id";

        // Выполняем DELETE-запрос к контроллеру
        ResultActions result = mockMvc.perform(delete("/admin/partitions/" + partitionIdToDelete)
                .contentType("application/json")
                .header("Authorization", "Bearer " + token));  // Добавляем токен в заголовок
